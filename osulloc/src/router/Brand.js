...
import BrandSwipe from './BrandSwipe';
    ...
    return (
        <div className='brand container mt-150 text' onMouseMove={handleMouseMove}>
            ...
            <ul className='jejuul position-relative mt-5 mb-5 pb-5'>
                ...
                <li className='jejuli ' onMouseOver={() => handleMouseOver(1)} onMouseLeave={() => handleMouseLeave(1)} data-aos="fade-left">
                    <span>향이 좋은 돌송이 차밭</span>
                    <p>돌송이 차밭 지역은 예로부터 화산재가 굳어서 돌멩이같이 잘게 부서진화산송이가 많아 ‘돌송이’라고 불리어 왔습니다. 이곳은 산과 바다를동시에 접하고 있어 매년 4월 한라산의 잔설을 품은 산바람과 바다의수분을 머금은 바닷바람이 밤낮으로 불어와 돌송이 차밭에서 채엽한차는 향이 매우 좋습니다.</p>
                    <div className='imgWrap' ref={(v) => (imgRefs.current[1] = v)}><img src="https://i.ibb.co/kh1Whwv/tea-field-item-2.jpg" alt="" /></div>
                </li>
                <li className='jejuli' onMouseOver={() => handleMouseOver(2)} onMouseLeave={() => handleMouseLeave(2)} data-aos="fade-right">
                    <span>맛이 좋은 한남 차밭</span>
                    <p>과거 ‘해들이밭’이라 하여 마을에서 가장 먼저 해가 드는 곳이라 불렸던한남 차밭은 온화한 기후에서 성장해 차나무가 어리고 아미노산 함량이높아 뛰어난 맛을 선사합니다.</p>
                    <div className='imgWrap' ref={(v) => (imgRefs.current[2] = v)}><img src="https://i.ibb.co/Mhf06fg/tea-field-item-3.jpg" alt="" /></div>
                </li>
            </ul>
            <BrandSwipe historyData={historyData.history}></BrandSwipe>
        </div>
    );
}
